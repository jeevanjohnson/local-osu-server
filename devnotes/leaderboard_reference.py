...
from objects.score import BanchoScore, Score
...
LB_NOT_SUPPORTED_PLACEHOLDER = Score(
    0, "", "Not supported atm!", 0, 0, 0, 0, 0, 0, 0, 0, False, 0, int(time.time())
)
BLANK_BMAP = Beatmap(
    approved=3,
    title_unicode="",
...
class Leaderboard:
    ...
    def as_binary(self) -> bytes:
        ...
        if self.personal_score:
            if self.personal_score not in self.scores:
                num_on_lb = glob.config.amount_of_scores_on_lb + 1
            else:
    ...
    @classmethod
    async def from_offline(cls, client_params: ParsedParams) -> "Leaderboard":
        lb = cls()
    ...
    @classmethod
    async def from_bancho(cls, client_params: ParsedParams) -> "Leaderboard":
        lb = cls()
        md5 = client_params["md5"]
        mods = client_params["mods"]
        rank_type = client_params["rank_type"]
        ...
        if glob.config.osu_api_key:
            params = {"k": glob.config.osu_api_key, "b": bmap.beatmap_id, "limit": 100}
            if rank_type == MODS:
                params["mods"] = mods

            async with glob.http.get(
                url=f"{OSU_API_BASE}/get_scores", params=params
            ) as resp:
                scores: list[SCORE] = [BanchoScore(x) for x in await resp.json()]
        else:
            scores: list[SCORE] = []

        if glob.config.pp_leaderboard:
            await bmap.get_file()
            scores.sort(key=lambda s: utils.calculator(s, bmap)[0], reverse=True)
