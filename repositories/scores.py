...
class ScoresRepository:
    ...
    async def get_scores_for_profile(self, profile_name: str) -> ScoresForProfile | None:
    ...
    async def get_scores_by_profile_and_beatmap_md5(
        ...
        async with self.scores as scores:
            ...
            return scores.profiles[profile_key].scores.get(
                beatmap_md5,
                MapScores(beatmap_md5=beatmap_md5, scores=[])
            )
            ...
            if game_mode is None:
                ...
                for map_scores in beatmaps_iter:
                    filtered = MapScores(
                        ...
                        scores=[s for s in map_scores.scores if s.game_mode == game_mode],
                    )
                    if filtered.scores:
