    ...
    async with aiohttp.ClientSession() as http:
        async with http.get("https://osu.ppy.sh/") as response:
            if response.status != 200:
                print(
                    f"Failed to fetch total registered users, status code: {response.status}"
                )
                return 0
            ...
            assert (
                user_count_element is not None and user_count_element.string is not None
...
async def get_50_ranks(page: int, game_mode: GameMode) -> None:
    ...
    async with aiohttp.ClientSession() as http:
        async with http.get(URL.format(mode=game_mode.value, page=page)) as response:
            if response.status != 200:
                print(
                    f"Failed to fetch page {page} for mode {game_mode.value}, status code: {response.status}"
                )
                return
            ...
            pp = [
                ...
                if row.string and "#" not in row.string
            ]

            for i, pp_value in enumerate(pp):
                DATA_POINTS[game_mode.value].append((rank_offset + i + 1, pp_value))
        ...
        with open(f"{TIME_STAMP}.json", "w") as f:
            f.write(json.dumps(DATA_POINTS, indent=4))
...
async def main():
        ...
        for page in range(1, pages + 1):
            while True:
                try:
                    ...
                    await asyncio.sleep(
                        2
                    )  # Sleep for 2 seconds between requests to avoid overwhelming the server
                    break
    ...
    for mode in GameMode:
        ...
        if len(data_points) >= 100:
            ...
            verify_pp = last_pp * math.exp(
                decay_per_rank_log * (TARGET_RANK - last_rank)
            )
            print(
            ...
            if DATA_POINTS[mode.value]:
                print(
                    f"  Final data range: rank {DATA_POINTS[mode.value][0][0]} to {DATA_POINTS[mode.value][-1][0]:,}"
                )
                print(
                    f"  Final PP range: {DATA_POINTS[mode.value][-1][1]} to {DATA_POINTS[mode.value][0][1]} PP"
                )
            print()
