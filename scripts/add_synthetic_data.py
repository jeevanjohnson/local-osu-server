...
def enhance_snapshot(snapshot_path: str, output_path: str = None, total_users: int = 27_001_016):
    """
    ...
    for mode_str in [GameMode.OSU.value, GameMode.TAIKO.value, GameMode.CATCH.value, GameMode.MANIA.value]:
        ...
        if len(data_points) < 100:
            print(f"Mode {mode_str}: Only {len(data_points)} points (need at least 100), skipping")
            continue
        ...
        print(f"  Final range: rank {data[mode_str][0][0]} to {data[mode_str][-1][0]:,}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print()
        print("Example:")
        print("  python add_synthetic_data.py usecases/domain/calculator/snapshots/20260328_033609.json")
